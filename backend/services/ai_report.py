...
Current price: ${stock_data.get('current_price', 0):.2f}"""
        ...
        prompt = f"""You are Warren Buffett analyzing a potential investment. Write exactly 3 paragraphs — no headers, no bullet points, plain prose only.
Paragraph 1 — Business Quality: Assess the business model, competitive moat, brand strength, pricing power, and revenue predictability based on the sector and financial trends.
Paragraph 2 — Financial Strength: Evaluate the company's profitability, free cash flow generation, return on equity, debt levels, and earnings consistency. Reference the specific numbers provided.
Paragraph 3 — Valuation Verdict: State whether this business is available at a fair price, a discount, or a premium. Reference the margin of safety percentage and give a clear Buy / Watchlist / Hold / Avoid recommendation with your reasoning.
Speak in first person as Warren Buffett. Be direct, opinionated, and reference specific financial figures. Do not hedge excessively.
Company: {stock_data.get('company_name', 'Unknown')} ({stock_data.get('ticker', 'N/A')}) — {stock_data.get('sector', 'Unknown')}
Scorecard: {scorecard.get('total_score', 0)}/100 ({scorecard.get('grade', 'N/A')})
Key metrics: Revenue growth {rev_growth_str}%, Net margin {net_margin_str}%, ROE {roe_str}%, Debt/Equity {de_str}
